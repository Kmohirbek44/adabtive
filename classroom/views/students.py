...
class QuizResultsView(View):
    ...
    def get(self, request, *args, **kwargs):        
        ...
        if not taken_quiz:
            """
           Не показывайте результат, если пользователь не пытался пройти тест
            """
...
def take_quiz(request, pk):
    ...
    if correct_answers == 3 and total_questions - total_unanswered_questions == 3:
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                if student.get_unanswered_questions(quiz).exists():
                    ...
                    student.save()
                    if percentage < 50.0:
                        messages.warning(request, 'В следующий раз повезет больше! Ваш результат в тесте %s был %s баллов' % (quiz.name, percentage))
                    else:
                        messages.success(request, 'Поздравляю! Вы завершили тест %s с успехом! Ты набрали %s болов.' % (quiz.name, percentage))
                    # return redirect('students:quiz_list')
    ...
    if correct_answers == 2 and total_questions - total_unanswered_questions == 3:
        congrations='Поздравляем, вы перешли на второй уровень сложности'
        return render(request, 'classroom/students/take_quiz_form.html', {
            ...
            'congrations':congrations

        })
    elif correct_answers == 6 and total_questions - total_unanswered_questions == 7:
        congrations = 'Поздравляем, вы перешли на третий уровень сложности'
        return render(request, 'classroom/students/take_quiz_form.html', {
            'quiz': quiz,
            'question': question,
            'form': form,
